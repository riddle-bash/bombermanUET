...
public class Doll extends Entity{
    ...
    public void update() {
        ...
        if (!broken) {
            if (x < x_goal) {
                temp_x = x + 1;
                if (!checkCollisionDoll(temp_x, y)) {
                    x ++;
            ...
            if (y < y_goal) {
                temp_y = y + 1;
                if (!checkCollisionDoll(x, temp_y)) {
                    y += 6;
                    this.img = Sprite.movingSprite(
                            Sprite.doll_right1,
                            Sprite.doll_right2,
                            Sprite.doll_right3,
                            BombermanGame.framePerSecond, 12).getFxImage();
                    return;
                }
            } else if (y > y_goal) {
                temp_y = y - 1;
    ...
    public boolean checkCollisionDoll(int oneal_x, int oneal_y) {
